...
@section('content')
<div x-data="chat()" class="h-[calc(100vh-10rem)] min-h-[520px] md:h-[600px] flex bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
    <!-- Conversations sidebar -->
    <div x-show="!isMobile || showConversations" x-transition.opacity class="w-full md:w-80 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <div
                @click="activeConv = {{ $i }}; if (isMobile) showConversations = false"
                :class="activeConv === {{ $i }} ? 'bg-indigo-50 dark:bg-indigo-900/30' : ''"
    ...
    <!-- Chat area -->
    <div x-show="!isMobile || !showConversations" x-transition.opacity class="flex-1 flex flex-col">
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <button x-show="isMobile" @click="showConversations = true" class="md:hidden p-2 -ml-2 rounded-lg text-gray-600 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700" aria-label="Retour aux conversations">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </button>
            <div class="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-bold" x-text="activeConversationInitial()"></div>
            <span class="font-semibold text-gray-900 dark:text-white" x-text="activeConversationName()"></span>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
            <div class="flex items-end gap-2">
                ...
                <div class="bg-gray-100 dark:bg-gray-700 rounded-xl rounded-bl-none px-4 py-2 max-w-[80%] md:max-w-xs">
            ...
            <div class="flex items-end gap-2 justify-end">
                <div class="bg-indigo-600 rounded-xl rounded-br-none px-4 py-2 max-w-[80%] md:max-w-xs">
        ...
        <div class="p-3 md:p-4 border-t border-gray-200 dark:border-gray-700 flex gap-2 md:gap-3">
            ...
            <button @click="sendMsg()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 md:px-4 py-2 rounded-xl transition text-sm">Envoyer</button>
        </div>
    </div>
</div>
...
function chat() {
    return {
        ...
        conversationNames: ['Kofi Anan', 'Aminata Diallo', 'Jean-Baptiste Kouassi', 'Fatou Touré', 'Equipe Lumo'],
        init() {
        ...
        activeConversationName() {
            return this.conversationNames[this.activeConv] ?? 'Conversation';
        },
        activeConversationInitial() {
            return this.activeConversationName().charAt(0).toUpperCase();
        },
        sendMsg() {
            this.$refs.msg.value = '';
        }
